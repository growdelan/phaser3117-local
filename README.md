...
## Print from Command-P

Install the per-user PDF menu action after installing the driver:

```sh
/bin/sh scripts/install-pdf-service.sh
```

Then, from an application's standard macOS print dialog:

1. Press **Command-P**.
2. Open the **PDF** menu at the bottom.
3. Choose **Drukuj na Xerox Phaser 3117** ("Print on Xerox Phaser 3117").

No manual PDF export or Terminal command is needed for each job. This uses
macOS PDF Services to hand the rendered document to our existing PDF helper.
The action prints one copy; print-dialog copy counts are not forwarded to the
helper. Page selection/layout depend on the PDF generated by the application.

**This is a PDF-menu action, not support for the ordinary Print button.**
On this macOS version, CUPS already sandboxes its filters and rejects a second
sandbox. The encoder therefore still runs before CUPS submission, in its own
mandatory deny-by-default sandbox. Ordinary direct queue submission remains
unsupported. No security setting or CUPS sandbox is disabled.

The per-user service is locally compiled from the readable AppleScript in
`macos/`. It does not run at login, monitor keys, or control other applications.
Its sole operation is passing each PDF path, shell-quoted, to the installed
helper. Some applications have a custom print preview; choose their system
print dialog to access the PDF menu. Close and reopen the print dialog after
...
If installed, also remove the per-user action without sudo:
